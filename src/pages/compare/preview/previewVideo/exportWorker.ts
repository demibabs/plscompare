...
async function runMediabunnyPipeline(config: ExportConfig) {
  ...
  try {
    const absoluteFontUrl = new URL("/fonts/Outfit-VariableFont_wght.ttf", self.location.origin).href;
  ...
  const streams = await Promise.all(sinks.map(async (sink, index) => {
  ...
  for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
    ...
    const sourcesDimensions = streams.map(s => s.currentSample ? {
    ...
    // C. Draw the layout
    renderFrame(ctx, canvasDimensions, layout, sources, sourcesDimensions, labelsText, timersText);
    ...
    sources.forEach((s) => { if (s) s.close(); });

    if (frameIndex % 10 === 0) {
      self.postMessage({ type: "PROGRESS", progress: (frameIndex / totalFrames) * 100 });
    }
  }
